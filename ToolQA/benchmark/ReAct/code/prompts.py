...
# REACT_INSTRUCTION = """Solve a question answering task with interleaving Thought, Action, Observation steps. Thought can reason about the current situation, and Action can be 13 types: 
# (1) Calculate[formula], which calculates the formula and returns the result.
# (2) LoadDB[DBName], which loads the database DBName and returns the database. The DBName can be one of the following: flights/coffee/airbnb/yelp.
# (3) FilterDB[condition], which filters the database DBName by the column column_name the relation (e.g., =, >, etc.) and the value value, and returns the filtered database.
# (4) GetValue[column_name], which returns the value of the column column_name in the database DBName.
# (5) Finish[answer], which returns the answer and finishes the task.
# The prefix of line type is added automatically, do not output it again.Strictly follow the pattern of one thought, one action and do not pretend to ouput observation.The first output is your thought.
# You may get no resluts after you filterd database,you can use LoadDB[DBName] to load database again.
# The numerical results are rounded to one decimal place.If it can be divided exactly, don't round it up or down
# Call the tool strictly following the format
# If you get many numbers to return and calculate them, you can distribute the calculations. For example, to calculate the average, first calculate the first 50, and then proceed to calculate the next 50
# You may take as many steps as necessary.
# Here are some examples:
# {examples}
# (END OF EXAMPLES)
...
(1) Calculate[formula], which calculates the formula and returns the result.
(2) LoadDB[DBName], which loads the database DBName and returns the database. The DBName can be one of the following: flights/coffee/airbnb/yelp.
(3) SQLInterpreter[SQL], which interprets the SQL query SQL and returns the result.
(4) Finish[answer], which returns the answer and finishes the task.
You may take as many steps as necessary.Use SQL query only.No other tools are allowed.
Here are some skills extracted from past experiences, which may help you to finish the task:
{rules}
Here are some examples:
{examples}
(END OF EXAMPLES)
The prefix of line type is added automatically, do not output it again.One thought ,one action and do not pretend to ouput observation.
You may get no resluts after you filterd database,you can use LoadDB[DBName] to load database again.
The numerical results are rounded to one decimal place.
Call the tool strictly following the format
Question: {question}{scratchpad}"""
